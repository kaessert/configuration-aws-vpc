# Iterate project

You are instructed to find the next important task in this project and work on it. 

- Start by verifying that what you are trying to implement is not already implemented
- Create a plan how to implement this feature:
  -> start with the spec
  -> develop tests
  -> implement code until the tests pass
- When you create tests, leave comments in the code which part of the spec they test,
  we ONLY want tests which verify the spec
- Once you finished implement the feature and the tests pass, commit your changes to the
  main branch

ONLY WORK AT ONE ITEM IN THE TASKLIST AT A TIME

If you're done with the task, mark it as done in the tasklist.

Remember at any point in time what the goal of this project is. Decide what has the highest
priority and take the SMALLEST POSSIBLE increment at a time.

Remember: The goal of this project is defined in CLAUDE.md and in thoughts/initial_prompt.md

IMPORTANT: Whenever you encounter issues, slow down, relax, think deeply what could have went
wrong and once you are able to resolve them leave notes for further agents in the thoughs folder.
